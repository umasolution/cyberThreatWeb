...
const useStyles = makeStyles((theme) => ({
  ...
  whiteBox: {
    ...
    borderTop: '0.5px solid #009ACD',
    borderRight: '0.5px solid  #009ACD',
    borderBottom: '0.5px solid #009ACD`',
    borderLeft: '0.5px solid #6C7B8B',
...
const ProjectsReports = () => {
  ...
  const [scanStatuses, setScanStatus] = useState([]);
  const onScan = async (e,scanData, rowIndex) => {
    console.log(rowIndex);
    e.stopPropagation();
    console.log(scanData);

    let scanStatusClone = [...scanStatuses];
    scanStatusClone[rowIndex] = "In-Progress";
    setScanStatus(scanStatusClone);
    ...
    let scanStatusNew = [...scanStatuses];
    scanStatusNew[rowIndex] = "Completed";
    setScanStatus(scanStatusNew);
    fetchProjectsList();
                                ...
                                <TableCell key={tabsData.columns[vkey].field} >
                                  ...
                                  {tabsData.columns[vkey].field == 'target_details' ? (
                                    <>
                                      ...
                                      {Object.entries(row.table[`${tabsData.columns[vkey].field}`]).map((target_details) => (
                                        Object.entries(target_details[1]).map((targetdata) => (
                                          (targetdata[0] == "System Name" || targetdata[0] == "Scan Project") ? (<>
                                            ...
                                            <Typography style={{ marginTop: '3px' }} className={classes.nameBoxContent} align="center" variant="caption" >
                                              {targetdata[1]}
                                            </Typography>
                                      ...
                                      <Grid container justify="flex-start" spacing={1} style={{ marginBottom: '3px' }} >
                                        {Object.entries(row.table[`${tabsData.columns[vkey].field}`]).map((target_details) => (
                                          Object.entries(target_details[1]).map((targetdata) => (
                                            (targetdata[0] != "System Name" && targetdata[0] != "Scan Project") ? (<>
