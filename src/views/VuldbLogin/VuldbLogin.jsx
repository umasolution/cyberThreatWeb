import {
  Container, Grid, LinearProgress, makeStyles, TextField, Typography, Box,
...
export const VuldbLogin = (/* {   } */) => {
  ...
  const callApi_browse = async () => {
    ...
    // Removing search by browse asd per new requirement       
    // var url = (searchtype == 'home') ? `/scan/home?${apiparams.toString()}` : `/scan/browse?${apiparams.toString()}`;  
    var url = (searchtype == 'home') ? `/scan/home?${apiparams.toString()}` : `/scan/home?${apiparams.toString()}`;
    let response = await Axios.get(url);
    if (response) {
      setTabsData(response.data);
      setPage(1);
  ...
  const handleChangePage = async (event, value) => {
    ...
    var url = '';
    if(searchtype == 'home'){
      url =  `/scan/home?${apiurl.toString()}`;
    }else if(searchtype == "browse"){
      url =  `/scan/browse?${apiurl.toString()}`;
    }
    let response = await Axios.get(url);
    setPage(value);
    setTabsData(response.data);
  ...
  const handleBrowseAdvanceClick = (event) => {
    ...
    } else if (searchBrowseType == 'language' ) {
      apiparams.set('type', searchBrowseType);
      apiparams.set('language', language);
      apiparams.set('year', year);
    } else if(searchBrowseType == 'platform' ){
      apiparams.set('type', searchBrowseType);
      apiparams.set(searchBrowseType, platform);
      apiparams.set('year', year);
    } else if(searchBrowseType == 'plugin'){
      apiparams.set('type', searchBrowseType);
      apiparams.set(searchBrowseType, plugin);
      apiparams.set('year', year);
    }
    ...
    if (!isEmpty(searchseverity)) {
      ...
      if (searchseverity == 1) {
        slidervalue = 'low';
        apiparams.set('severity', slidervalue);
    ...
    if (!isEmpty(searchVector)) {
      if(searchVector.indexOf('remote') != -1)
       apiparams.set('remote', true);

      if(searchVector.indexOf('local') != -1)
       apiparams.set('local', true);
    }
  ...
  const onClickLink = async (payload) => {
    ...
    //  url = url + '?product=' + payload.product + '&type=product&producttype=' + payload.producttype;
        url = url + '?type=product'+'&product='+ payload.product;
      response = await Axios.get(url);

    } else if (payload.vendor) {
     // url = url + '?vendor=' + payload.vendor + '&type=vendor';
      url = url + '?type=vendor'+'&vendor='+ payload.vendor;
      response = await Axios.get(url);

    } else {
     // url = url + '?vulnerabilties=' + payload.cwe_text + '&type=vulnerabilties';
     url = url + '?type=vulnerabilties'+'&vulnerabilties='+ payload.cwe_text.split('-')[1];
      response = await Axios.get(url);
                        ...
                        (<Grid container spacing={3}>
                          ...
                          <Grid item xs={7} >
                            ...
                            >
                              <List>
                                <ListItem>
                                  <RadioGroup aria-label="browseradio" style={{ display: 'inline' }} name="browseradio" value={searchBrowseType} onChange={(e) => handleBrowseRadio(e)}>
                                  ...
                                  {searchBrowseType == 'language' ? getLanguageControls() : ''}
                                  {searchBrowseType == 'platform' ? getPlatformControls() : ''}
                                  {searchBrowseType == 'plugin' ? getPluginControls() : ''}
                                </ListItem>
                              </List>
                            </Box>
                          </Grid>
  ...
  const getProductControls = () => {
    return (
      <Grid container spacing={1}>
        ...
        <Grid md={12} item >
          <div style={{ display: 'flex' }}>
            <label style={{ margin: '5px',marginRight:'30px', marginTop: '16px', fontWeight: '600', fontSize: '14px' }}>Type:</label>
            <Select
              id="type"
              label="Type"
              value={typeData}
        ...
        <Grid md={12} item style={{marginLeft : '10px'}}>
          <div style={{ display: 'flex' }}>
            <label style={{ margin: '5px',marginRight:'22px', marginTop: '16px', fontWeight: '600', fontSize: '14px' }}>Year:</label>
            <Select
              id="year"
              label="Years"
              value={year}
  ...
  const getLanguageControls = () => {
    return (
      <Grid container >
        <Grid md={4} item >
          <div style={{ display: 'flex' }}>
            <label style={{ margin: '5px', marginTop: '16px', fontWeight: '600', fontSize: '14px' }}>Language:</label>
            <TextField
              name="language"
              type="language"
  ...
  const getPlatformControls = () => {
    return (
      <Grid container >
        <Grid md={4} item >
          <div style={{ display: 'flex' }}>
            <label style={{ margin: '5px', marginTop: '16px', fontWeight: '600', fontSize: '14px' }}>Platform:</label>
            <TextField
              name="platform"
              type="platform"
  ...
  const getPluginControls = () => {
    return (
      <Grid container >
        <Grid md={4} item >
          <div style={{ display: 'flex' }}>
            <label style={{ margin: '5px', marginTop: '16px', fontWeight: '600', fontSize: '14px' }}>Plugin:</label>
            <TextField
              name="plugin"
              type="plugin"
              placeholder="Plugin"
