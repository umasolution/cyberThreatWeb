...
export const Search = (/* {   } */) => {
    ...
    const fetchFeed = async () => {
        try {
            ...
            if(keyword){
               ...
               setKeywordData(`Keyword : ${keyword}`);
               apiparams.set('keyword', keyword);
            } else if(product){
               url = `/search?product=${product}`;
               setKeywordData(`Product : ${product}`);
               apiparams.set('product', product);
            } else if (vendor){
               url = `/search?vendor=${vendor}`;
               setKeywordData(`Vendor : ${vendor}`);
               apiparams.set('vendor', vendor);
            } else if (product && vendor) {
               url = `/search?product=${product}&vendor=${vendor}`;
               apiparams.set('product', product);
               apiparams.set('vendor', vendor);
            }
            setMainUrl('/search');
   ...
   const cvesearchcenter = (tabsData,col) => {
                    ...
                    {issearch?(<>
                      ...
                      <TableRow>
                          <TableCell key='active'>
                             <Skeleton animation="wave" height="20px" cvenoresult width="100%" />
                          </TableCell>
                          <TableCell key='active'>
                            <Skeleton animation="wave" height="20px" cvenoresult width="100%" />
                          </TableCell>
                      ...
                      <TableRow>
                          <TableCell><Skeleton animation="wave" height="40px" cvenoresult width="100%" /></TableCell>
                          <TableCell><Skeleton animation="wave" height="40px" cvenoresult width="100%" /></TableCell>
                          <TableCell><Skeleton animation="wave" height="40px" cvenoresult width="100%" /></TableCell>
                          <TableCell><Skeleton animation="wave" height="40px" cvenoresult width="100%" /></TableCell>
                          <TableCell><Skeleton animation="wave" height="40px" cvenoresult width="100%" /></TableCell>
    ...
    return (
      <>
      <Container maxWidth="false" className="cveresult">
        <Grid
            container
            spacing={3}
